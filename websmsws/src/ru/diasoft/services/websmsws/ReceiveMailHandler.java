...
public class ReceiveMailHandler {
	...
	public ReceiveMailHandler() {
		Map<String, Object> mailBoxesParams = (Map<String, Object>)Config.getConfig("websmsws").getAllParams().get(MAIL_BOXES_PARAM);
		if (mailBoxesParams == null || mailBoxesParams.size() == 0) {
	 ...
	 * @param ReceiverEmail
	 * @param MailMessageDecodeFlag	  - не обязательный параметр флаг декодирования письма. true -  
	 * 
	 * @param params
	 * @return
	 * @throws ReceiveMailHandlerException
	...
	/**
	 * Base64 преобразует в текст.
	 * Входящие параметры текст для преобразования и кодировка.
	 * 
	 * @param cx
	 * @param thisObj
	 * @param args
