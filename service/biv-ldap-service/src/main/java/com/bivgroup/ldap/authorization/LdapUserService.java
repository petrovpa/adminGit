...
public class LdapUserService {
    ...
    private static final String[] BASE_USER_PARAMETERS = new String[]{"memberof", "name", "userPrincipalName", "sAMAccountName", "givenName",
                                                                      "sn", "userAccountControl", "distinguishedName"};
    ...
    private ActiveDirectoryUserInfo searchUserByCondition(InitialDirContext context, SearchActiveDirectoryUserCondition condition) throws ServiceException {
        ...
        try {
            ...
            if (answers.hasMore()) {
                SearchResult answerElement = answers.nextElement();
                Attributes answerElementAttributes = answerElement.getAttributes();
                final String distinguishedName = answerElementAttributes.get("distinguishedName").toString();
                final List<String> filterDistinguishedName = getExcludeDistinguishedName();
                if(allowedGroupToSignIn(distinguishedName, filterDistinguishedName)) {
    ...
    /**
     * Метод проверки разрешенности входа пользователя по группам в дереве Active Directory из конфига
     *
     * @param distinguishedName         - полученное значение поля - distinguishedName - из свойств Active Directory по пользователю
     * @param filterDistinguishedName   - значения групп в дереве Active Directory для фильтрации
     *
     * @return булевное значение о разрешенности входа пользователя по группам в дереве Active Directory из конфига
     */
    private boolean allowedGroupToSignIn(final String distinguishedName, final List<String> filterDistinguishedName) {

        boolean toAllow = true;

        if (filterDistinguishedName != null) {
            for(String element: filterDistinguishedName) {
                if (distinguishedName.indexOf(element) != -1) {
                    toAllow = false;
                    break;
                }
            }
    ...
    private List<String> getExcludeDistinguishedName() {
        ...
        return validGroups == null ? null : Arrays.asList(validGroups.split(";"));
    }
    ...
    /**
     * Метод анализа группы пользователя в AD
     *
    ...
    private List<String> getValidUserGroups() {
        String validUserGroups = config.getParam("validUserGroups", "Пользователи удаленного рабочего стола;Администраторы домена");
        return Arrays.asList(validUserGroups.split(";"));
    }
