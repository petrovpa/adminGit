package com.bivgroup.service.login.admin.service;
...
@Path("/rest/login")
public class LoginAdminGate extends LoginBase {
    private boolean isNeedCheckAdminRight;

    public LoginAdminGate() {
        super();
    ...
    protected String checkAuthorization(AuthUserInfo authUserInfo) {
        ...
        if (isNeedCheckAdminRight) {
            ...
            if (error.isEmpty() && !getBooleanParam(rightCheckResult, "ACCESS", false)) {
                error = "У пользователя нет прав администратора для входа в данный проект.";
            }
        }
        return error;
    }
